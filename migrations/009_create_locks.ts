...
export async function createLocksCollection(client: Client): Promise<void> {
	...
	await databases.createIndex(DATABASE_ID, LOCKS_COLLECTION_ID, "campaign_idx", "key", [
		"campaignId",
	]);

	// Create index on expiresAt for stale lock cleanup
	await databases.createIndex(DATABASE_ID, LOCKS_COLLECTION_ID, "expires_idx", "key", [
		"expiresAt",
	]);
