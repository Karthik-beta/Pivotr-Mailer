...
async function setupIndexes(databases: Databases) {
	...
	await safelyCreateIndex(CollectionId.LEADS, "unsubscribed_idx", IndexType.Key, [
		"isUnsubscribed",
	]);
	await safelyCreateIndex(CollectionId.LEADS, "campaign_queue_idx", IndexType.Key, [
		"campaignId",
		"status",
		"queuePosition",
	]);
...
async function seedInitialData(databases: Databases) {
	...
	try {
		...
		console.log("Global metrics exist.");
	}
}
