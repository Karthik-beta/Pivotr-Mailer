 ...
 */
export async function acquireCampaignLock(client: Client, campaignId: string): Promise<LockResult> {
	...
	try {
		...
		const existingLocks = await databases.listDocuments(DATABASE_ID, LOCKS_COLLECTION_ID, [
		...
		// Create new lock
		const lockDoc = await databases.createDocument(DATABASE_ID, LOCKS_COLLECTION_ID, ID.unique(), {
			campaignId,
			instanceId,
 ...
 */
export async function refreshCampaignLock(client: Client, lockId: string): Promise<boolean> {
	const databases = new Databases(client);
	const now = new Date();
	const expiresAt = new Date(now.getTime() + LOCK_TTL_SECONDS * 1000);

	try {
		await databases.updateDocument(DATABASE_ID, LOCKS_COLLECTION_ID, lockId, {
			expiresAt: expiresAt.toISOString(),
		});
		return true;
	} catch {
 ...
 */
export async function releaseCampaignLock(client: Client, lockId: string): Promise<boolean> {
	const databases = new Databases(client);
...
export async function cleanupStaleLocks(client: Client): Promise<number> {
	...
	try {
		const staleLocks = await databases.listDocuments(DATABASE_ID, LOCKS_COLLECTION_ID, [
