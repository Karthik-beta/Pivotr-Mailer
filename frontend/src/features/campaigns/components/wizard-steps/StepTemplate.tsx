...
export function StepTemplate({ data, onChange, errors }: StepTemplateProps) {
    ...
    const resolvedSignOff = useMemo(() => {
        ...
        const resolved = replaceVariables(resolveSpintax(data.template.signOff.content, refreshKey > 0));
        return parseMarkdown(resolved);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const updateSignOff = (updates: Partial<NonNullable<CampaignFormData['template']['signOff']>>) => {
        onChange({
            template: {
                ...data.template,
                signOff: {
    ...
    const updateSocialLink = (index: number, updates: Partial<NonNullable<CampaignFormData['template']['signOff']>['socialLinks'][0]>) => {
        const links = [...(data.template.signOff?.socialLinks || [])];
        links[index] = { ...links[index], ...updates } as NonNullable<CampaignFormData['template']['signOff']>['socialLinks'][0];
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent className="max-w-xs">
                                    ...
                                    <p className="text-xs mt-1">
                                        Use {'{{FirstName}}'} for lead variables.
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
...
🌐 [www.acme.com](https://acme.com)"
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        Full Markdown support: # Heading, **bold**, *italic*, [links](url), ![images](url), lists, quotes, and more.
                                    </p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {data.template.signOff?.media?.map((media, index) => (
                                        <div key={index} className="flex gap-2 items-start p-3 rounded-lg bg-muted/50">
                                            <Select
                                                value={media.type}
                                                onValueChange={(value: 'image' | 'gif' | 'logo') => updateMedia(index, { type: value })}
                                                disabled={!data.template.signOff?.enabled}
                                            >
                                            ...
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <div className="grid grid-cols-3 gap-2">
                                                    ...
                                                    <Input
                                                        type="number"
                                                        value={media.width || ''}
                                                        onChange={(e) => updateMedia(index, { width: parseInt(e.target.value) || undefined })}
                                                        placeholder="Width"
                                                        className="text-xs"
                                                        disabled={!data.template.signOff?.enabled}
                                                    />
                                    ...
                                    {data.template.signOff?.socialLinks?.map((link, index) => {
                                        ...
                                        return (
                                            <div key={index} className="flex gap-2 items-center">
                                                <Select
                                                    value={link.platform}
                                                    onValueChange={(value: 'linkedin' | 'twitter' | 'website' | 'email' | 'phone') =>
                                                ...
                                                <Input
                                                    ...
                                                    placeholder={link.platform === 'email' ? 'email@example.com' : link.platform === 'phone' ? '+1 234 567 8900' : 'https://...'}
                ...
                <Card className="bg-muted/50">
                    <CardHeader className="pb-2 border-b space-y-1">
                        ...
                        <div className="font-semibold pt-2">
                            {resolvedSubject || 'Subject preview...'}
                        </div>
                    </CardHeader>
                    <CardContent className="pt-4 space-y-4">
                        ...
                        {data.template.signOff?.enabled && (
                            <div className="border-t pt-4 space-y-3">
                                ...
                                {data.template.signOff.socialLinks && data.template.signOff.socialLinks.length > 0 && (
