...
export function StepDelay({ data, onChange, errors }: StepDelayProps) {
    const updateDelayConfig = (field: keyof CampaignFormData['delayConfig'], value: number | boolean) => {
        onChange({
            delayConfig: {
                ...data.delayConfig,
                [field]: value,
    ...
    return (
        <div className="space-y-6">
            <p className="text-sm text-muted-foreground">
                Configure the delay between sending emails. A Gaussian distribution ensures
                human-like, natural timing patterns that help with email deliverability.
            </p>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Switch
                            ...
                            onCheckedChange={(checked) =>
                                updateDelayConfig('gaussianEnabled', checked)
                            }
                        />
                    </div>
                </div>
