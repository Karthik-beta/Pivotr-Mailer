...
import { useAssignLeads, usePreviewLeads } from "../hooks/useCampaigns";
...
export function AssignLeadsDialog({ open, onOpenChange, campaignId }: AssignLeadsDialogProps) {
	...
	}, [leadTypes, leadStatuses, maxLeads, open, previewMutation.mutate]);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>Select criteria to add more leads to this campaign.</DialogDescription>
				</DialogHeader>
						...
						<div className="flex flex-wrap gap-4">
							{LEAD_TYPES.map(({ value, label }) => (
								<div key={value} className="flex items-center gap-2">
									...
									/>
									<Label htmlFor={`assign-type-${value}`} className="cursor-pointer">
										{label}
									</Label>
								</div>
							))}
						...
						<div className="flex flex-wrap gap-4">
							{LEAD_STATUSES.map(({ value, label }) => (
								<div key={value} className="flex items-center gap-2">
									...
									/>
									<Label htmlFor={`assign-status-${value}`} className="cursor-pointer">
										{label}
									</Label>
								</div>
							))}
						...
						<CardContent className="pt-4">
							<div className="flex justify-between items-center">
								...
								{isLoadingPreview ? (
									...
									<span className="text-sm text-muted-foreground">Select criteria</span>
								) : (
									<span className="font-bold font-mono">{previewCount.toLocaleString()}</span>
								)}
							</div>
						</CardContent>
					</Card>
