	...
	const parseCSV = (text: string): StageLeadsRequest["leads"] => {
		...
		for (let i = 1; i < lines.length; i++) {
			...
			const lead: StageLeadsRequest["leads"][0] = {
				fullName:
					values[headers.indexOf("fullname")] || values[headers.indexOf("name")] || values[0],
				email: values[headers.indexOf("email")] || values[1],
				companyName:
					values[headers.indexOf("companyname")] || values[headers.indexOf("company")] || values[2],
				phoneNumber:
					values[headers.indexOf("phonenumber")] || values[headers.indexOf("phone")] || values[3],
			};
			...
			<div className="space-y-6">
				...
				<Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "review" | "import")}>
					<TabsList className="grid w-full max-w-md grid-cols-2">
						<TabsTrigger value="review">Review Leads ({stats.total})</TabsTrigger>
						<TabsTrigger value="import">Import New Leads</TabsTrigger>
					</TabsList>
							...
							<CardContent>
								{stats.invalid > 0 && (
									<Alert variant="destructive" className="mb-4 bg-red-50 border-red-200">
										...
										<AlertDescription>
											{stats.invalid} lead(s) have validation errors. Click on a lead to expand and
											see details, then use the edit button to correct the data.
										</AlertDescription>
									</Alert>
								)}
